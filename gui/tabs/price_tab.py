...
class PriceTab:
    ...
    def plot_trading_signals(self, ax, data, indicators, dates):
        ...
        ax.set_title('Торговые сигналы и индикаторы', fontsize=12, fontweight='bold', pad=10)
        
        # Настройка временной шкалы
        self.setup_time_axis(ax, dates, self.timeframe_var.get())
        
        # Основной торговый сигнал
        ...
        if 'trend_strength' in data.columns:
            trend_axis = ax.twinx()
            trend_axis.plot(dates, data['trend_strength'], label='Сила тренда', 
                        color='red', linewidth=1, alpha=0.7)
            trend_axis.set_ylabel('Сила тренда', color='red', labelpad=5, fontsize=8)
            trend_axis.set_ylim(0, 1)
            trend_axis.legend(loc='upper right', fontsize=6)
        ...
        if self.show_rsi.get():
            ...
            if rsi_data is not None:
                rsi_axis = ax.twinx()
                # Смещаем ось RSI если есть сила тренда
                if trend_axis is not None:
                    rsi_axis.spines['right'].set_position(('outward', 40))
                rsi_axis.plot(dates, rsi_data, label='RSI', 
                            color='blue', linewidth=1, alpha=0.7)
                rsi_axis.axhline(y=70, color='red', linestyle='--', alpha=0.5)
                rsi_axis.axhline(y=30, color='green', linestyle='--', alpha=0.5)
                rsi_axis.set_ylabel('RSI', color='blue', labelpad=5, fontsize=8)
                rsi_axis.set_ylim(0, 100)
                rsi_axis.legend(loc='lower right', fontsize=6)
        
        # Настройка основного графика
        ax.set_xlabel('Дата', labelpad=5, fontsize=9)
        ax.set_ylabel('Сигнал', labelpad=5, fontsize=9)
        ...
        ax.autoscale(enable=True, axis='x', tight=True)  # Минимум отступов по X
        
        # УВЕЛИЧИВАЕМ ОТСТУПЫ ВНУТРИ ГРАФИКА
        current_ylim = ax.get_ylim()
        ax.set_ylim(-0.1, 1.1)  # Фиксированные границы вместо расчетных
        
        # УВЕЛИЧИВАЕМ ОТСТУПЫ ПОДПИСЕЙ
        ax.set_xlabel('Дата', labelpad=8, fontsize=9)    # Было 5 → стало 8
        ax.set_ylabel('Сигнал', labelpad=8, fontsize=9)
        
        if self.show_trade_signals.get():
            ax.legend(loc='upper left', fontsize=7, bbox_to_anchor=(0, 0.98))  # Смещаем от верха
        
        # 🔍 ДИАГНОСТИКА ОТСТУПОВ СИЛЫ ТРЕНДА
        if trend_axis is not None:
            print("🔍 ДИАГНОСТИКА СИЛЫ ТРЕНДА:")
            print(f"   Положение оси: {trend_axis.get_position()}")
            print(f"   Y limits силы тренда: {trend_axis.get_ylim()}")
            print(f"   Подпись Y: '{trend_axis.get_ylabel()}'")
            
            # Проверяем границы подписи
            ylabel_obj = trend_axis.yaxis.get_label()
            print(f"   Отступ подписи: {ylabel_obj.get_position()}")
            print(f"   Выравнивание подписи: {ylabel_obj.get_ha()}, {ylabel_obj.get_va()}")
            
        if rsi_axis is not None:
            print(f"   Положение оси RSI: {rsi_axis.get_position()}")
        
        print(f"   Общие границы ax2: {ax.get_position()}")
        print(f"   X limits основного графика: {ax.get_xlim()}")
        print(f"   Y limits основного графика: {ax.get_ylim()}")
    ...
    def update_plot(self, result_name: str):
        ...
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(15, 16), 
                                   gridspec_kw={'height_ratios': [1.2, 1]})  # Больше места си
        ...
        plt.subplots_adjust(
            ...
            top=0.96,     # Увеличиваем отступ сверху
            hspace=0.2    # Увеличиваем расстояние между графиками
        )
        ...
        # ВРЕМЕННО: проверяем границы
        print(f"Границы ax2: {ax2.get_position()}")
        print(f"X limits: {ax2.get_xlim()}")
        print(f"Y limits: {ax2.get_ylim()}")
        
        # Дополнительная диагностика после построения
        print("🔍 ДИАГНОСТИКА ПОСЛЕ ПОСТРОЕНИЯ:")
        print(f"   Границы ax2: {ax2.get_position()}")
        
        # Проверяем все оси на графике сигналов
        for i, axis in enumerate(ax2.figure.axes):
            if axis != ax1 and axis != ax2:  # Только дополнительные оси
                print(f"   Доп.ось {i}: {axis.get_position()}")
                if hasattr(axis, 'get_ylabel'):
                    print(f"     Подпись: {axis.get_ylabel()}")
