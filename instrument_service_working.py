...
class InstrumentServiceWorking:
    ...
    def search_instruments_working(self, query):
        ...
        try:
            with self._get_client() as client:
                ...
                for instrument in result.instruments:
                    try:
                        ...
                        # Безопасное преобразование типа инструмента в строку
                        try:
            ...
            # Игнорируем ошибки связанные с типами инструментов
            if "is not a valid InstrumentType" not in str(e):
...
if __name__ == "__main__":
    ...
    # Тест получения по FIGI
    print("\n🔎 Тест получения по FIGI...")
    instrument = service.get_instrument_by_figi_working('BBG004730N88')
    if instrument and hasattr(instrument, 'instrument'):
        instr = instrument.instrument
        print(f"✅ {instr.ticker}: {instr.name}")
        print(f"   Лот: {instr.lot}, Валюта: {instr.currency}")
    
    # Тест расписания
    print("\n📅 Тест расписания торгов...")
    schedules = service.get_trading_schedules_working()
    if schedules:
        print(f"Получено расписание для {len(schedules.exchanges)} бирж")